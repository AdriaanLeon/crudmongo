...
router.post('/', async (req, res) => {
    const { title, author, genre, publication_dat } = req?.body
    if (!title || !author || !genre || !publication_dat) {
...
router.put('/:id', getBook, async (req, res) => {
    try {
        ...
        book.publication_dat = req.body.publication_dat || book.publication_dat;
...
router.patch('/:id', getBook, async (req, res) => {
    ...
    if (!req.body.title && !req.body.author && !req.body.genre && !req.body.publication_dat) {
        res.status(400).json({
